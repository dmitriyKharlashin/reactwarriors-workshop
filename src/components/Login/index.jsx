...
export default class LoginForm extends Component {
  ...
  onSubmit = event => {
    ...
    if (this.validateFields ()) {
      ...
      fetch (
        `${API_MOVIE_DB_URL}/authentication/token/new?${prepareGetParams (params)}`
      )
        ...
        .then (data => {
          ...
          fetch (
            `${API_MOVIE_DB_URL}/authentication/token/validate_with_login?${prepareGetParams (
              {
                api_key: API_KEY_3,
  ...
  validateFields () {
    ...
    Object.keys (this.state).forEach (field => {
      ...
      if (!this.state[field]) {
        errorsArray[field] =
          field[0].toUpperCase () + field.slice (1, field.length) + ' is empty';
      }
    });
    ...
    return (
      <div className="form-login-container">
        <form className="form-login">
          ...
          {errors.global &&
            <div className="invalid-feedback">{errors.global.message}</div>}
          <button
            type="submit"
